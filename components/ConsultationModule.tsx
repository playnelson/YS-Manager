...
export const ConsultationModule: React.FC = () => {
  ...
  return (
    <div className="h-full flex gap-2 overflow-hidden">
      ...
      <div className="w-20 flex flex-col gap-2 win95-raised p-2 shrink-0 overflow-y-auto">
        <NavButton
      ...
      {/* Área Principal */}
      <div className="flex-1 win95-sunken border-2 border-palette-mediumLight dark:border-gray-800 overflow-hidden relative">
...
const NavButton = ({ active, onClick, icon, label }: any) => (
  <button
    onClick={onClick}
    className={`w-full aspect-square flex flex-col items-center justify-center gap-1 p-1 border-2 transition-all ${active ? 'border-black dark:border-white bg-palette-lightest dark:bg-gray-800 shadow-[inset_1px_1px_0px_#000] text-blue-700 dark:text-blue-400' : 'win95-raised hover:bg-palette-mediumLight dark:hover:bg-gray-700 text-palette-darkest/70 dark:text-gray-400'}`}
  >
    {icon}
...
const CnpjTool: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      <div className="win95-raised p-3 flex gap-2 items-end">
        <div className="flex-1">
          <label className="text-[10px] font-bold uppercase block mb-1">Digite o CNPJ</label>
          <input className="w-full win95-sunken p-1.5 font-mono font-bold text-palette-darkest dark:text-white" value={cnpj} onChange={e => setCnpj(formatCnpj(e.target.value))} placeholder="00.000.000/0001-00" onKeyDown={e => e.key === 'Enter' && handleSearch()} />
        </div>
      ...
      {data && (
        <div className="flex-1 bg-palette-lightest dark:bg-gray-900 win95-sunken p-4 overflow-y-auto">
          <div className="flex justify-between items-start mb-4 border-b pb-2">
            <div className="flex-1 pr-4">
              <h2 className="text-lg font-black uppercase text-blue-800 dark:text-blue-400 leading-tight select-text cursor-text">{data.razao_social}</h2>
              <p className="text-xs font-bold text-palette-darkest/50 select-text cursor-text">{data.nome_fantasia}</p>
            </div>
          ...
          <div className="flex gap-2 mb-4">
            <Button size="sm" onClick={() => setTab('resumo')} className={tab === 'resumo' ? 'bg-palette-mediumLight dark:bg-blue-900' : ''}>Resumo</Button>
            <Button size="sm" onClick={() => setTab('socios')} className={tab === 'socios' ? 'bg-palette-mediumLight dark:bg-blue-900' : ''}>Sócios ({data.qsa?.length || 0})</Button>
          </div>
          ...
          {tab === 'resumo' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="col-span-full bg-blue-50 dark:bg-blue-900/30 p-2 border border-blue-100 dark:border-blue-800">
                <span className="text-[9px] font-bold uppercase text-blue-800 dark:text-blue-400">Atividade Principal (CNAE)</span>
                <div className="font-bold text-sm text-palette-darkest dark:text-white select-text cursor-text">{data.cnae_fiscal} - {data.cnae_fiscal_descricao}</div>
              </div>
            </div>
          )}
          ...
          {tab === 'socios' && (
            <div className="space-y-2">
              {data.qsa?.map((socio, i) => (
                <div key={i} className="win95-raised bg-palette-mediumLight/50 dark:bg-gray-800 p-2">
                  <div className="font-bold text-sm text-palette-darkest dark:text-white select-text cursor-text">{socio.nome_socio}</div>
                  <div className="text-xs text-palette-darkest/60 select-text cursor-text">{socio.qualificacao_socio}</div>
                </div>
              ))}
              {(!data.qsa || data.qsa.length === 0) && <div className="text-center italic text-palette-darkest/50">Sem sócios listados ou Capital Aberto.</div>}
            </div>
          )}
        </div>
      )}
...
const NFeTool: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col p-4 bg-win95-bg bg-transparent">
      <div className="win95-raised p-4 max-w-2xl mx-auto w-full">
          ...
          <div className={`win95-sunken bg-palette-lightest dark:bg-gray-900 p-4 border-l-4 ${info.valida ? 'border-l-green-600' : 'border-l-red-600'}`}>
            ...
            <div className="grid grid-cols-2 gap-4 text-xs">
              ...
              <div className="p-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                ...
                <span className="font-bold text-gray-900 dark:text-white">{info.periodo}</span>
              </div>
              <div className="p-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 col-span-2">
                <span className="text-[9px] font-bold text-gray-500 uppercase block">CNPJ do Emitente</span>
                <span className="font-bold font-mono text-sm text-gray-900 dark:text-white select-text cursor-text">{info.cnpj}</span>
              </div>
              <div className="p-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
                <span className="text-[9px] font-bold text-gray-500 uppercase block">Modelo</span>
                <span className="font-bold text-gray-900 dark:text-white">{info.modelo}</span>
              </div>
              <div className="p-2 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
...
const CepTool: React.FC = () => {
  ...
  return (
    <div className="h-full p-4 flex flex-col items-center">
      <div className="max-w-md w-full win95-raised p-4">
        <h3 className="font-bold mb-2 flex items-center gap-2"><MapPin size={16} /> Buscar Endereço</h3>
        <div className="flex gap-2 mb-4">
          <input className="flex-1 win95-sunken p-2 font-mono text-palette-darkest dark:text-white" placeholder="00000-000" value={cep} onChange={e => setCep(e.target.value)} />
        ...
        {data && (
          <div className="win95-sunken bg-palette-lightest dark:bg-gray-900 p-4 space-y-2">
            ...
            <div className="text-sm text-palette-darkest/50 select-text">{data.cep}</div>
...
const BanksTool: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col gap-2 p-2">
      <div className="win95-raised p-2 flex gap-2 items-center">
      ...
      <div className="flex-1 win95-sunken bg-palette-lightest dark:bg-gray-900 overflow-y-auto">
        <table className="w-full text-xs text-left">
          <thead className="bg-palette-mediumLight dark:bg-gray-800 sticky top-0 font-bold uppercase text-palette-darkest/70 dark:text-gray-300">
            <tr>
              <th className="p-2 border-r dark:border-gray-700">Código</th>
              <th className="p-2 border-r dark:border-gray-700">ISPB</th>
              <th className="p-2">Instituição</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map(b => (
              <tr key={b.ispb} className="hover:bg-palette-mediumLight/50 dark:hover:bg-blue-900/20 border-b dark:border-gray-800">
                <td className="p-2 font-mono font-bold text-palette-darkest dark:text-blue-400 text-center select-text">{b.code}</td>
                <td className="p-2 font-mono text-center text-palette-darkest/50 select-text">{b.ispb}</td>
                <td className="p-2 text-palette-darkest dark:text-white select-text">{b.name}</td>
              </tr>
            ))}
          </tbody>
        </table>
