        ...
        <div className="flex-1 win95-sunken bg-white overflow-auto">
          <table className="w-full text-left text-xs border-collapse">
            <thead className="sticky top-0 bg-win95-bg z-10 border-b-2 border-win95-shadow">
              <tr>
                <th className="px-4 py-2 font-bold uppercase text-[#555] border-r border-white w-1/3">Nome / Colaborador</th>
                <th className="px-4 py-2 font-bold uppercase text-[#555] border-r border-white w-1/4">Departamento</th>
                <th className="px-4 py-2 font-bold uppercase text-[#555] border-r border-white w-1/4">Ramal / Telefone</th>
                <th className="px-4 py-2 font-bold uppercase text-[#555] text-right">Ações</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {filteredExtensions.length === 0 ? (
                ...
                filteredExtensions.map(ext => (
                  <tr key={ext.id} className="hover:bg-blue-50 group transition-colors">
                    ...
                    <td className="px-4 py-2 font-mono text-win95-blue font-bold border-r border-gray-50 text-sm tracking-wider">
                      {ext.number}
                    </td>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/40 z-[100] flex items-center justify-center p-4">
          <div className="bg-win95-bg w-full max-w-sm win95-raised p-1 shadow-2xl">
            <div className="bg-win95-blue text-white px-2 py-1 text-sm font-bold flex justify-between items-center mb-2 select-none">
            ...
            <form onSubmit={handleSave} className="p-4 space-y-4">
              ...
              <div className="flex gap-2 pt-2">
                ...
                <Button type="submit" className="flex-1 bg-win95-blue text-white">GRAVAR DADOS</Button>
              </div>
            </form>
          </div>
        </div>
