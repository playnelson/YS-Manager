          ...
          <div className="h-full flex flex-col md:flex-row gap-0 overflow-hidden bg-[#525659]">
            ...
            <div className="w-full md:w-80 bg-win95-bg border-r-2 border-black/30 flex flex-col shrink-0 z-20 shadow-2xl">
              ...
              <div className="flex-1 overflow-y-auto p-5 space-y-6 custom-scrollbar bg-[#d4d0c8]">
                {/* BrasilAPI Intelligence Integration */}
                <div className="bg-slate-800 p-4 rounded-lg shadow-inner mb-6 border border-white/10 group">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="text-[9px] font-black uppercase text-blue-400 flex items-center gap-2">
                      <Sparkles size={12} /> Brasil Intelligence I/O
                    </h4>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 win95-sunken bg-slate-900 border-none text-white text-[10px] px-3 py-2 outline-none font-mono placeholder:text-slate-600"
                      placeholder="CNPJ ou CEP para Auto-Preencher..."
                      onKeyDown={async (e) => {
                        if (e.key === 'Enter') {
                          const val = (e.target as HTMLInputElement).value.replace(/\D/g, '');
                          if (val.length === 8 || val.length === 14) {
                            const type = val.length === 8 ? 'cep' : 'cnpj';
                    ...
                    <button
                      onClick={async (e) => {
                        const input = document.getElementById('intelligence-search') as HTMLInputElement;
                        const val = input.value.replace(/\D/g, '');
                        if (!val) return;

                        const isCnpj = val.length === 14;
                        const isCep = val.length === 8;

                        if (!isCnpj && !isCep) {
                          alert('Formato inválido. Use CNPJ (14 dígitos) ou CEP (8 dígitos).');
                          return;
                        }
                        ...
                        try {
                          const endpoint = isCnpj ? `cnpj/v1/${val}` : `cep/v2/${val}`;
                          ...
                          if (isCnpj) {
                            if (newValues.hasOwnProperty('OutorganteFull')) newValues['OutorganteFull'] = data.razao_social;
                            if (newValues.hasOwnProperty('ClienteNome')) newValues['ClienteNome'] = data.razao_social;
                            if (newValues.hasOwnProperty('EmpresaNome')) newValues['EmpresaNome'] = data.razao_social;
                            if (newValues.hasOwnProperty('CPF_CNPJ')) newValues['CPF_CNPJ'] = data.cnpj;
                            if (newValues.hasOwnProperty('Endereço')) {
                              newValues['Endereço'] = `${data.logradouro}, ${data.numero}, ${data.bairro}, ${data.municipio}-${data.uf}`;
                            }
                          } else {
                            if (newValues.hasOwnProperty('Endereço')) {
                              newValues['Endereço'] = `${data.street}, ${data.neighborhood}, ${data.city}-${data.state}`;
                            }
                            if (newValues.hasOwnProperty('Cidade')) newValues['Cidade'] = data.city;
                            if (newValues.hasOwnProperty('Localidade')) newValues['Localidade'] = data.city;
                          }

                          setFormValues(newValues);
                          input.value = '';
                          input.placeholder = 'DADOS IMPORTADOS!';
                          setTimeout(() => input.placeholder = 'CNPJ ou CEP para Auto-Preencher...', 2000);
                        } catch (err) {
                          alert('Erro ao buscar dados na BrasilAPI.');
                        } finally {
                          btn.disabled = false;
                          btn.innerHTML = 'SYNC';
                        }
                      }}
                      className="win95-raised px-3 bg-blue-600 text-white text-[9px] font-black uppercase hover:bg-blue-700 active:translate-y-px transition-all shrink-0"
                    >
                      SYNC
                    </button>
                  </div>
